...
def save_images(sampled_imgs, label, directory):
    b_size,temp,temp2,temp3=sampled_imgs.shape
    sampled_images=sampled_imgs.reshape(b_size,temp2*temp2)
    sio.savemat(os.path.join(directory, '{}.mat'.format(label)), {'images': sampled_images})


    
def print_images2(b_size, sampled_images, label, index, directory, save_all_samples=False):
    ...
    if type(sampled_images) == np.ndarray:
        ...
        #b_size,temp,temp2,temp3=sampled_images.shape
        
        sampled_images=sampled_images.reshape(b_size,28,28,1)
        N, h, w, cdim = [b_size, 28, 28, 1]
...
class SynthDataset2():    
    def __init__(self, x_dim=100, num_clusters=10, seed=1234):
        ...
        arr1=np.random.multivariate_normal(mean1, cov1, int(self.N/3))
        arr2=np.random.multivariate_normal(mean2, cov2, int(self.N/3))
        arr3=np.random.multivariate_normal(mean3, cov3, int(self.N/3))
...
class SynthDataset3():
    ...
    def __init__(self, n_mixture=9, std=0.01, radius=1.0):
        self.x_dim=2
        self.N=10000
        ...
        self.newdata = data.sample(self.N)
        with tf.Session() as sess:
            self.X=sess.run(self.newdata)
...
class SynthDataset4():
    ...
    def __init__(self, num_components=25, std=0.01, radius=1.0):
        self.x_dim=2
        self.N=10000
        cat = ds.Categorical(tf.zeros(num_components,dtype=float))
        import itertools
        mus = np.array([np.array([i, j]) for i, j in itertools.product(range(-4, 5, 2),
                                                               range(-4, 5, 2))],dtype=float)
        s = 0.05
        sigmas = [np.array([s,s]).astype(dtype=float) for i in range(num_components)]
...
class MnistDataset():
    ...
    def __init__(self, data_dir):
        ...
        self.N = self.mnist.train.images.shape[0]
...
class FMnistDataset():
    ...
    def __init__(self, data_dir):
        ...
        from tensorflow.examples.tutorials.mnist import input_data
        self.mnist = fmnist = input_data.read_data_sets('data/fashion', source_url='http://fashion-mnist.s3-website.eu-central-1.amazonaws.com/')
        self.x_dim = [28, 28, 1]
        self.num_classes = 10
        self.dataset_size = self.mnist.train.images.shape[0]
