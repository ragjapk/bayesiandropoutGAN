    ...
    with tf.variable_scope(scope or "Linear"):
        if matrix is None:
            matrix = tf.get_variable("Matrix", [shape[1], output_size], tf.float32,
                                     tf.random_normal_initializer(stddev=1. / tf.sqrt(shape[1] / 2.)))
        if bias is None:
...
def dropout(x, dropout_rate, name, training):
    return tf.layers.dropout(x, dropout_rate, training = training)
